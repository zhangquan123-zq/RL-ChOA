...
switch F
    %% 焊接梁设计问题(Welded Beam Design, WBD)
    case 'F1'
        fobj = @F1;
    ...
    %% 压缩弹簧设计问题(Compression Spring Design, CSD)
    case 'F2'
        fobj = @F2;
    ...
    %% 压力容器设计问题(Pressure Vessel Design, PVD)
    case 'F3'
        fobj = @F3;
        lb = [0; 0; 10; 10];           % 下界
        ub = [99; 99; 200; 200];        % 上界
...
PCONST = 100000;          % 罚函数常数 PENALTY FUNCTION CONSTANT
...
G1 = TAU-TAUMAX;         % 最大切应力约束 MAX SHEAR STRESS CONSTRAINT
G2 =  SIGMA-SIGMAX;     % 最大弯曲应力约束 MAX BENDING STRESS CONSTRAINT
