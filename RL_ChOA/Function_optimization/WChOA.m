...
function [Attacker_score, Attacker_pos, Convergence_curve] = WChOA(X, N, Max_iter, lb, ub, dim, fobj)
...
while l < Max_iter
    ...
    for i = 1:N
        for j = 1:dim
            ...
            X(i, j) = 1./(w1+w2+w3+w4).*(w1.*X1+w2.*X2+w3.*X3+w4.*X4)/4;% Equation (8)
            
        end
    end
    l = l+1;
    Convergence_curve(l) = Attacker_score;
    %disp(['WChOA: At iteration ', num2str(l), ' ,the best fitness is ', num2str(Convergence_curve(l))]);
end